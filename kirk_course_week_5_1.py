...
sw1_graph=nx.Graph()
# add nodes
net_nodes=cdp_output.keys()
sw1_graph.add_nodes_from(net_nodes)
# add adges
net_edges=[]
for node in cdp_output.keys():
    for neigh in cdp_output[node]['neighbors'].keys():
        edge=(node,neigh)
        net_edges.append(edge)
sw1_graph.add_edges_from(net_edges)

print sw1_graph.nodes()
print sw1_graph.edges()

nx.draw_networkx(sw1_graph,node_size=100,font_size=18,with_labels=True)
plt.savefig('topology4.png')
plt.show()
